...
class MobileUser:
    ...
    def registration(self, home_agent, foreign_agent):
        ...
        s = int(input('MU chooses a random number : ')     )
        EID = xor(hash(xor(self.IDmu,self.PWmu,True)) ,s, True)
        print("EID: \n", EID)
        S = home_agent.user_registration_2(EID)

        print("\nStep 3")        
        self.SPW = xor(xor(S,hash(self.PWmu)), hash(orutil(self.IDmu, self.s)))
        print("SPW: \n", self.SPW)
        self.s1 = xor(self.s, hash(orutil(self.IDmu, self.PWmu)))
    ...
    def aesk(self, home_agent, foreign_agent):
        ...
        s = xor(self.s1, hash(orutil(self.IDmu, self.PW1mu)))
        self.EID1 = xor(hash(xor(self.IDmu,self.PW1mu)), s)

        print("EID1: \n", self.EID1)

        self.S1 = xor(xor(self.SPW, hash(self.PW1mu)), hash(orutil(self.IDmu, s)))
        self.EIDnew = xor(hash(xor((self.IDmu), self.PW1mu)), self.snew)
        self.Vm = xor(self.EIDnew, hash(orutil(self.S1,self.Nm)))
        self.s1new = xor(self.snew, hash(orutil(self.IDmu, PW1mu)))
        ...
        foreign_agent.aesk_step2(home_agent, self.EID1, self.Vm, self.Qm, self.Nm)
